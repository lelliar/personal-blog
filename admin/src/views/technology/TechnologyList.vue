  ...
  <div>
       <el-form class="center" style="opacity:0.8;flex-direction:column;" @submit.native.prevent="save">
           <el-form-item label="标题:" class='sty' style="margin-top:60px;">
            ...
            <el-form-item label="分类:"  class='sty'>
               <el-input style="width:300px;" v-model="data.category"></el-input>
           </el-form-item>
            <el-form-item label="作者:"  class='sty'>
               <el-input style="width:300px;" v-model="data.name"></el-input>
           </el-form-item>
           ...
           <div style="margin-left:60%;margin-top:-43%">
                <el-form-item label="上传背景图:"  class='sty' style="margin-left:30px;height:400px;">
                    <el-upload
...
export default {
    ...
    methods: {
       async fetch(){
        const res = await this.$http.get(`technology/${this.id}`)
        this.data = res.data
      },
        async save(){
            if(this.id){
                 await this.$http.put(`technology/${this.id}` , this.data)
                
            }
            else{
               await this.$http.post('technology' , this.data)
            }
            this.$router.push('/technology/list')
            this.$message({
                type:'success',
                message:'保存成功'
            })
